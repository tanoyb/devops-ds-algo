...
func RecursiveDelete(node *Node, key int) *Node {
	...
	if key < node.data {
		...
		//the node has been found/reached
		if GenNodeHeight(node.left) > GenNodeHeight(node.right) {
			//take the inorder predecessor
	...
	if BalanceFactor(node) == 2 && BalanceFactor(node.left) == 1 { // L1 Rotation
		return LL_Rotation(node)
	} else if BalanceFactor(node) == 2 && BalanceFactor(node.left) == -1 { // L-1 Rotation
		return LR_Rotation(node)
	} else if BalanceFactor(node) == -2 && BalanceFactor(node.right) == -1 { // R-1 Rotation
		return RR_Rotation(node)
	} else if BalanceFactor(node) == -2 && BalanceFactor(node.right) == 1 { // R1 Rotation
		return RL_Rotation(node)
	} else if BalanceFactor(node) == 2 && BalanceFactor(node.left) == 0 { // L0 Rotation
		return LL_Rotation(node)
	} else if BalanceFactor(node) == -2 && BalanceFactor(node.right) == 0 { // R0 Rotation
		return RR_Rotation(node)
	}
